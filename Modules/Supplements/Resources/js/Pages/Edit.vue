<template>
    <AppLayout title="Editar Suplemento">
      <template #header>
          ...
          <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <form @submit.prevent="submit" class="p-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div v-if="supplement.media && supplement.media.length" class="col-span-2">
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Archivos Multimedia Existentes</label>
                    ...
                    <div 
                      ...
                      <div class="flex items-center">
                        <div class="flex-shrink-0 h-10 w-10 bg-gray-200 dark:bg-gray-600 rounded flex items-center justify-center">
                          <svg v-if="isImage(mediaItem)" class="h-6 w-6 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="col-span-2">
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Agregar Nuevos Archivos</label>
              ...
              <div class="mt-6 flex justify-end space-x-3">
                ...
                <button 
                  ...
                  {{ form.processing ? 'Guardando...' : 'Actualizar Suplemento' }}
                </button>
              </div>
            </form>
          </div>
      ...
      <ConfirmationModal 
        :show="confirmingMediaDeletion" 
        @close="confirmingMediaDeletion = false">
        <template #title>
          ¿Eliminar Archivo?
        </template>
        <template #content>
        ...
        <template #footer>
          <button 
            @click="confirmingMediaDeletion = false" 
  ...
  const form = useForm({
    ...
    instructions: props.supplement.instructions || '',
    default_price: props.supplement.default_price,
    price: props.supplement.price,
    status: props.supplement.status,
    media: [], // Solo para nuevos archivos
    _method: 'PUT',
  });
  ...
  const mediaIndexToDelete = ref(null);
  ...
  const removeExistingMedia = (index) => {
    mediaIndexToDelete.value = index;
    confirmingMediaDeletion.value = true;
  };
  
  const deleteMediaConfirmed = () => {
    router.post(route('supplements.remove-media', props.supplement.id), {
      index: mediaIndexToDelete.value,
      _method: 'POST',
    }, {
      preserveScroll: true,
      onSuccess: () => {
        confirmingMediaDeletion.value = false;
        mediaIndexToDelete.value = null;
      },
    });
  };
  ...
  const submit = () => {
    form.post(route('supplements.update', props.supplement.id), {
      preserveScroll: true,
      onSuccess: () => {
        mediaFiles.value = [];
      },
