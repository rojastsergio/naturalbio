<template>
    <AppLayout title="Editar Terapia">
      <template #header>
...
const form = useForm({
  ...
  default_price: props.therapy?.default_price || 0.00,
  price: props.therapy?.price || null,
  default_duration: props.therapy?.default_duration || 60,
  media: props.therapy?.media || null,
  status: props.therapy?.status || 'active',
});

const submit = () => {
  form.put(route('therapies.update', props.therapy.id), {
    onSuccess: () => {
