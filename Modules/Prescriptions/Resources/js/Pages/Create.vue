<template>
  ...
  const form = useForm({
    patient_id: props.selectedPatient ? props.selectedPatient.id : '',
    doctor_id: props.selectedDoctor ? props.selectedDoctor.id : '',
    issue_date: new Date().toISOString().slice(0, 10), // Today's date
    expiry_date: null,
    diagnosis: '',
    instructions: '',
    items: [],
  ...
  onMounted(() => {
    // Add at least one empty item when the component is mounted
    if (form.items.length === 0) {
      addItem();
    }
  });
